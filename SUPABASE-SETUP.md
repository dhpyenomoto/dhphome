...
## 5.5 写真アップロードを有効にする（Supabase Storage）

管理画面から **カバー写真・ギャラリー写真をアップロード**するには、画像保存用の
公開バケットを1回だけ用意します（GitHubトークンは不要になります）。

1. 左メニュー **SQL Editor → New query**
2. リポジトリの **`supabase-storage.sql`** の中身を貼り付けて **Run**
   - 公開バケット `images` と、公開読み取り／ログイン書き込みの権限が作られます
   - （ダッシュボードの **Storage → New bucket → 名前 `images` → Public** でも可）
3. これで、管理画面でログイン中に「**カバー写真をアップロード**」や
   ギャラリーの「写真を選択」を押すと、Supabase に保存され URL が自動設定されます。
