...
案件追加は簡易化しています。必須は **ID（自動採番）・カテゴリー・名称(日本語)** のみで、
英語は自動翻訳で補完、**土地規模・計画概要・ストーリー・写真・資料は折りたたみ（任意）**。
**「保存して公開」** ボタンで「英語の自動翻訳 → 下書き反映 → GitHub へコミット」を一括実行します
（複数案件は「下書きに追加」を繰り返し、最後にまとめて保存も可能）。
...
- 編集フォーム上部の「日本語の入力→英語を自動翻訳」をオンにすると、日本語項目から
  フォーカスを外した際に、対応する英語項目が**空のとき**だけ自動翻訳して入力します。
- 「未入力の英語をまとめて翻訳」ボタンで一括翻訳も可能です。
